...
## What it does
- `grab_story.js` uses Playwright to scrape pages (text + image URLs) from a Gemini share into `data/story.json`.
- `download_images.ps1` saves the referenced images to `images/`.
- TTS options:
...
- Outputs: images go to `images/`, audio to `audio/`, per-page videos to `video/`, final render to `output/story.mp4`.
